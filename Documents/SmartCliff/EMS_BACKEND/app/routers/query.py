...
router_ = APIRouter(prefix="/admin/queries", tags=["Admin Queries"])

# Get all queries (optionally filtered by status)
@router_.get("/", response_model=List[QueryResponse])
async def get_queries(status: Optional[str] = None, db: Session = Depends(db.get_db)):
...
async def respond_query(query_id: int, response: str = Form(...), db: Session = Depends(db.get_db)):
...
async def close_query(query_id: int, db: Session = Depends(db.get_db)):
...
async def view_query(query_id: int, db: Session = Depends(db.get_db)):
