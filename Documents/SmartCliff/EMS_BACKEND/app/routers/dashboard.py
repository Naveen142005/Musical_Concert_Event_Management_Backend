...
def get_dashboard_overview(db_session: Session = Depends(db.get_db)):
...
def get_recent_activities(limit: int = 10, db_session: Session = Depends(db.get_db)):
...
# def get_analytics(period: str = "week", db_session: Session = Depends(db.get_db)):
